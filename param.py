...
#            ( 't_pole', 'temperature at the poles t_pole = 0.981 compares to -5.0°C compares to 253.15 K', 'double', 0.981 ),
...
#            ( 'c_land', 'water vapour reduction on land ( 55% of the saturation value )', 'double', 0.55 ),
    ...
    def write_pxd ( filename, model, sections ):
        with open ( filename, 'w' ) as f:
            ...
            f.write ( """# pxd files\n""" )
            if len(sys.argv)>1 and sys.argv[1]=='mchin_dev':
                f.write ( """# THIS FILE IS AUTOMATICALLY GENERATED BY param.py
# ANY CHANGES WILL BE OVERWRITTEN AT COMPILE TIME
cdef extern from "c%sModelEx.h":
    cppclass c%sModel:
