...
void Results_Hyd::run_data ( int i_beg, double dr, double dthe, double dphi, double L_hyd, double u_0,
...
// printout of surface data at one predefinded location
    ...
    Value_3 = EkmanPumping.y[ j_loc ][ k_loc ];
...
// calculation of the ratio ocean to land, also addition and substraction of CO2 of land, ocean and vegetation
    ...
    for ( int j = 0; j < jm; j++ ){
        for ( int k = 0; k < km; k++ ){
            if ( AtomUtils::is_land( h, im-1, j, k) )        h_land++;
        }
    }
