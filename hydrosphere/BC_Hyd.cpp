...
*/
            u.x[0][j][k] = u.x[3][j][k] 
                - 3. * u.x[2][j][k] + 3. * u.x[1][j][k];  // extrapolation
            u.x[im-1][j][k] = u.x[im-4][j][k] 
                - 3. * u.x[im-3][j][k] + 3. * u.x[im-2][j][k];  // extrapolation
            v.x[0][j][k] = v.x[3][j][k] 
                - 3. * v.x[2][j][k] + 3. * v.x[1][j][k];  // extrapolation
            v.x[im-1][j][k] = v.x[im-4][j][k] 
                - 3. * v.x[im-3][j][k] + 3. * v.x[im-2][j][k];  // extrapolation
            w.x[0][j][k] = w.x[3][j][k] 
                - 3. * w.x[2][j][k] + 3. * w.x[1][j][k];  // extrapolation
            w.x[im-1][j][k] = w.x[im-4][j][k] 
                - 3. * w.x[im-3][j][k] + 3. * w.x[im-2][j][k];  // extrapolation
