...
//	IC_water = .01;				// no dimension, ( average velocity compares to          u_0 * IC_water = 0,45 * IC_water = 0,0045 m/s )
	IC_water = .03;				// no dimension, ( average velocity compares to          u_0 * IC_water = 0,45 * IC_water = 0,0135 m/s )
...
// temperature-distribution by Ruddiman approximated by a parabola
	...
	c_average = ( t_average + 346. ) / 10.;									// in psu
	...
	if (  Ma >= 0 )																				// when time slices are run, modern world excluded
	{
		for ( int k = 0; k < km; k++ )
		{
			for ( int j = 0; j < jm; j++ )
			{
				...
				{
					d_j = ( double ) j;
					t.x[ im-1 ][ j ][ k ] = t_coeff * ( d_j * d_j / ( d_j_half * d_j_half ) - 2. * d_j / d_j_half ) + t_pole + t_cretaceous; // paleo surface temperature
...
void BC_Thermohalin::IC_Atlantischer_Ozean ( Array &h, Array &u, Array &v, Array &w, Array &c )
{
...
void BC_Thermohalin::IC_Indischer_Ozean ( Array &h, Array &u, Array &v, Array &w )
{
// Ströme entlang der Küsten
// Schliessen der polaren, subpolaren und subtropischen atmosphärischen Wirbelsysteme
...
void BC_Thermohalin::IC_Pazifischer_Ozean ( Array &h, Array &u, Array &v, Array &w )
{
...
void BC_Thermohalin::IC_Nord_Polar_Meer ( Array &h, Array &u, Array &v, Array &w )
{
// Ströme entlang der Küsten
// Schliessen der polaren, subpolaren und subtropischen atmosphärischen Wirbelsysteme
...
void BC_Thermohalin::IC_South_Polar_Sea ( Array &h, Array &u, Array &v, Array &w, Array &c )
{
// flow along coasts
		...
		for ( int j = j_beg; j < j_end + 1; j++ )
		{
			for ( int k = k_beg; k < k_end; k++ )
			{
				...
				if ( h.x[ i ][ j ][ k ] == 0. )
				{
					...
					v.x[ i ][ j ][ k ] =  .5 * IC_water * ( v.x[ i ][ j_end ][ k ] - v.x[ i ][ j_beg ][ k ] ) * ( double ) ( j_z ) / ( double ) ( j_n ) + v.x[ i ][ j_beg ][ k ];
//					w.x[ i ][ j ][ k ] = .5 * IC_water * ( w.x[ i ][ j_end ][ k ] - w.x[ i ][ j_beg ][ k ] ) * ( double ) ( j_z ) / ( double ) ( j_n ) + w.x[ i ][ j_beg ][ k ];
				}
			}
		}
	}
...
//					v.x[ i ][ j ][ k ] = - IC_water * ( double ) ( i - i_beg + 5 ) / ( double ) ( i_max - i_beg + 5 );
					w.x[ i ][ j ][ k ] = .5 * IC_water * ( double ) ( i - i_beg + 5 ) / ( double ) ( i_max - i_beg + 5 );
				}
			}
		}
	}
	...
	for ( int i = 10; i < im-2; i++ )
	{
		for ( int j = j_beg; j < j_end; j++ )
		{
			for ( int k = k_beg; k < k_end; k++ )
			{
				...
				{
					c.x[ i ][ j ][ k ] = ca_max;
					v.x[ i ][ j ][ k ] = + .5 * IC_water * ( double ) ( i - i_beg ) / ( double ) ( i_max - i_beg );
					w.x[ i ][ j ][ k ] = + .5 * IC_water * ( double ) ( i - i_beg ) / ( double ) ( i_max - i_beg );
				}
			}
		}
	}
	...
	for ( int i = 10; i < im-2; i++ )
	{
		for ( int j = j_beg; j < j_end; j++ )
		{
			for ( int k = k_beg; k < k_end; k++ )
			{
				...
				{
					c.x[ i ][ j ][ k ] = ca_max;
					v.x[ i ][ j ][ k ] = .5 * IC_water * ( v.x[ i ][ j ][ k_end ] - v.x[ i ][ j ][ k_beg ] ) * ( double ) ( k_z ) / ( double ) ( k_n ) + v.x[ i ][ j ][ k_beg ];
					w.x[ i ][ j ][ k ] = .5 * IC_water * ( w.x[ i ][ j ][ k_end ] - w.x[ i ][ j ][ k_beg ] ) * ( double ) ( k_z ) / ( double ) ( k_n ) + w.x[ i ][ j ][ k_beg ];
				}
			}
		}
	}
	...
	for ( int i = 10; i < im-2; i++ )
	{
		for ( int j = j_beg; j < j_end; j++ )
		{
			for ( int k = k_beg; k < k_end; k++ )
			{
				...
				{
					c.x[ i ][ j ][ k ] = ca_max;
					v.x[ i ][ j ][ k ] = + .5 * IC_water * IC_water * ( double ) ( i - i_beg ) / ( double ) ( i_max - i_beg );
					w.x[ i ][ j ][ k ] = + .5 * IC_water * ( double ) ( i - i_beg ) / ( double ) ( i_max - i_beg );
				}
			}
		}
...
void BC_Thermohalin::IC_EquatorialCurrents ( Array &h, Array &u, Array &v, Array &w )
{
	...
	for ( int i = i_EIC_u; i < i_EIC_o; i++ )
	{
		for ( int j = 88; j < 93; j++ )
		{
			for ( int k = 145; k < 271; k++ )
			{
				...
				{
					v.x[ i ][ j ][ k ] = 0.;
					w.x[ i ][ j ][ k ] = - .05 * IC_water * ( double ) ( i - i_EIC_u ) / ( double ) ( i_EIC_o - i_EIC_u );
				}
			}
		}
	}
	...
	for ( int i = i_SCC_u; i < i_SCC_o; i++ )
	{
		for ( int j = 86; j < 88; j++ )
		{
			for ( int k = 135; k < 271; k++ )
			{
				...
				{
					v.x[ i ][ j ][ k ] = 0.;
					w.x[ i ][ j ][ k ] = .05 * IC_water * ( double ) ( i - i_SCC_u ) / ( double ) ( i_SCC_o - i_SCC_u );
				}
			}
		}
	}
	...
	for ( int i = i_ECC_u; i < i_ECC_o; i++ )
	{
		for ( int j =83; j < 88; j++ )
		{
			for ( int k = 135; k < 271; k++ )
			{
				...
				{
					v.x[ i ][ j ][ k ] = 0.;
					w.x[ i ][ j ][ k ] = + .05 * IC_water * ( double ) ( i - i_ECC_u ) / ( double ) ( i_ECC_o - i_ECC_u );
				}
			}
		}
	}
	...
	for ( int i = i_SCC_u; i < i_ECC_o - 1; i++ )
	{
		for ( int j = 83; j < 88; j++ )
		{
			for ( int k = 265; k < 276; k++ )
			{
				...
				{
					{
						u.x[ i ][ j ][ k ] = + .01 * IC_water * ( double ) ( i - i_SCC_u ) / ( double ) ( i_ECC_o - 1 - i_SCC_u );
						v.x[ i ][ j ][ k ] = - .05 * IC_water * ( double ) ( i - i_SCC_u ) / ( double ) ( i_ECC_o - 1 - i_SCC_u );
//						w.x[ i ][ j ][ k ] = - .05 * IC_water * ( double ) ( i - i_SCC_u ) / ( double ) ( i_ECC_o - 1 - i_SCC_u );
					}
				}
			}
		}
...
void BC_Thermohalin::IC_DeepWater ( Array &h, Array &u, Array &v, Array &w, Array &c )
{
		...
		for ( int j = 32; j < 37; j++ )
		{
			for ( int k = k1; k < k3; k++ )
			{
				if ( h.x[ i ][ j ][ k ] == 0. )
				{
					...
					c.x[ i ][ j ][ k ] = ( ca_max - ca ) * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - kn ) / ( double ) ( kd ) + ca;
					u.x[ i ][ j ][ k ] = - .1 * IC_water * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - kn ) / ( double ) ( kd );
				}
			}
		}
	}
		...
		for ( int j = 32; j < 51; j++ )
		{
			for ( int k = k1; k <= k3; k++ )
			{
				if ( h.x[ i ][ j ][ k ] == 0. )
				{
					...
					c.x[ i ][ j ][ k ] = ( ca_max - ca ) * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - kn ) / ( double ) ( kd ) + ca;
					v.x[ i ][ j ][ k ] = + .5 * IC_water * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - kn ) / ( double ) ( kd );
				}
			}
		}
	}
			...
			for ( int k = ( k_beg + k_run ); k < ( k_beg + k_step + k_run ); k++ )
			{
				for ( int i = i_bottom; i < i_deep + 1; i++ )
				{
					if ( h.x[ i ][ j ][ k ] == 0. )
					{
						...
						c.x[ i ][ j ][ k ] = ( ca_max - ca ) * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - kn ) / ( double ) ( kd ) + ca;
						v.x[ i ][ j ][ k ] = + .5 * IC_water * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - kn ) / ( double ) ( kd );
						w.x[ i ][ j ][ k ] = - .5 * IC_water * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - kn ) / ( double ) ( kd );
					}
				}
			}
		k_run++;
		...
		for ( int j = 60; j < 89; j++ )
		{
			for ( int k = k1; k <= k3; k++ )
			{
				if ( h.x[ i ][ j ][ k ] == 0. )
				{
					...
					c.x[ i ][ j ][ k ] = ( ca_max - ca ) * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - kn ) / ( double ) ( kd ) + ca;
					v.x[ i ][ j ][ k ] = + IC_water * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - kn ) / ( double ) ( kd );
				}
			}
		}
	}
		...
		for ( int j = j1; j <= j3; j++ )
		{
			for ( int k = 193; k < 286; k++ )
			{
				if ( h.x[ i ][ j ][ k ] == 0. )
				{
					...
					c.x[ i ][ j ][ k ] = ( ca_max - ca ) * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - jn ) / ( double ) ( jd ) + ca;
					w.x[ i ][ j ][ k ] = - IC_water * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - jn ) / ( double ) ( jd );
				}
			}
		}
	}
		...
		for ( int j = j1; j <= j3; j++ )
		{
			for ( int k = 171; k < 210; k++ )
			{
				if ( h.x[ i ][ j ][ k ] == 0. )
				{
					...
					c.x[ i ][ j ][ k ] = ( ca_max - ca ) * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - jn ) / ( double ) ( jd ) + ca;
					u.x[ i ][ j ][ k ] = + .1 * IC_water * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - kn ) / ( double ) ( kd );
					w.x[ i ][ j ][ k ] = + .5 *  IC_water * ( double ) ( i - i_bottom ) / ( double ) ( i_deep - i_bottom ) * ( double ) ( - jn ) / ( double ) ( jd );
				}
			}
		}
	}
...
/*
	for ( int k = 0; k < km; k++ )
	{
		for ( int j = 0; j < jm; j++ )
		{
//			p_j.y[ j ][ k ]  =  ( rR * t.x[ i ] [ j ][ k ] * ( t_0 + 20. ) ) / p_0;		// pressure distribution on the surface in hPa / hPa
			p_j.y[ j ][ k ]  =  1.;		// pressure distribution on the surface in hPa / hPa
			p_dyn.x[ im-1 ][ j ][ k ] = p_dyn.x[ im-1 ][ j ][ k ];

			if ( h.x[ im-1 ][ j ][ k ] == 1. )
			{
				p_dyn.x[ im-1 ][ j ][ k ]  =  0.;																				//  pressure distribution in solid surroundings
				p_dyn.x[ im-1 ][ j ][ k ] = p_dyn.x[ im-1 ][ j ][ k ];
			}
		}
	}
*/
	...
	for ( int k = 0; k < km; k++ )
	{
		for ( int j = 0; j < jm; j++ )
		{
			...
			{
				if ( h.x[ i ][ j ][ k ] == 1. )   p_dyn.x[ i ][ j ][ k ] = pa;
				else
				{
					d_i = ( double ) i;
					p_dyn.x[ i ][ j ][ k ] = ( rg * ( d_i_max - d_i ) / 100. + p_0 ) / p_0;				// step size in 150 m   given in hPa / hPa
				}
			}
		}
	}
