...
// calculation of a total quantity as sum on all values in a virtual column in r-direction
	...
	for ( int k = 0; k < km; k++ )
	{
		for ( int j = 0; j < jm; j++ )
		{
			p_baro = ( r_0_air * 287.1 * t_j.y[ j ][ k ] * ( t_0 + 20. ) ) / 100.;											// surface pressure from barometric formula in hPa
			...
			e_SL = ( r_0_water_vapour * R_WaterVapour * t_j.y[ j ][ k ] * t_0 ) * .01;
			a_SL = 216.6 * e_SL / ( t_j.y[ j ][ k ] * t_0 );																		// absolute humidity in kg/m3

			c_grad = ( - 3. * c_j.y[ j ][ k ] + 4. * cc.x[ 1 ][ j ][ k ] - cc.x[ 2 ][ j ][ k ] ) / ( 2. * dr );				// water vapour pressure gradient in Kg/(Kg m)
			t_grad = ( - 3. * t_j.y[ j ][ k ] + 4. * tc.x[ 1 ][ j ][ k ] - tc.x[ 2 ][ j ][ k ] ) / ( 2. * dr );					// temperature gradient in K/m
			...
			if ( hc.x[ 0 ][ j ][ k ] == 0. )
			{
				...
				Q_sen.y[ j ][ k ] = - r_0_air * cp_l * coeff_Diffusion_sensibel * t_j.y[ j ][ k ] * t_0 * t_grad / L_atm;	// sensible heat in [W/m2] from energy transport equation
			}
			else 
			{
				Q_Bal.y[ j ][ k ] = 0.;
	...
	for ( int k = 0; k < km; k++ )
	{
		for ( int j = 0; j < jm; j++ )
		{
//			prec.y[ j ][ k ] = ( rai.y[ j ][ k ] + rai_su.y[ j ][ k ] + ice.y[ j ][ k ] ) * coeff_mmWS;	// precipitation consists of 100% saturated water vapour, supercooled water, ice
