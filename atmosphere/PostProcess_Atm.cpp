...
void PostProcess_Atmosphere::Atmosphere_v_w_Transfer ( string &Name_Bathymetry_File, double u_0, Array &v, Array &w, Array &t, Array &p_dyn, Array_2D &Evaporation_Penman, Array_2D &Precipitation )
{
    ...
    for ( int j = 0; j < jm; j++ ) {
        for ( int k = 0; k < km; k++ ) {
            v_w_Transfer_File << v.x[ 0 ][ j ][ k ] * u_0 << " " << w.x[ 0 ][ j ][ k ] * u_0 << " " << t.x[ 0 ][ j ][ k ] << " " << p_dyn.x[ 0 ][ j ][ k ] << " " << Evaporation_Penman.y[ j ][ k ] << " " << Precipitation.y[ j ][ k ] << endl;	// dimensional v and w values
        }
    }
