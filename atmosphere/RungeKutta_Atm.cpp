    ...
    for(int i = 1; i < im-1; i++){
        for(int j = 1; j < jm-1; j++){
            for(int k = 1; k < km-1; k++){
                ...
                t.x[i][j][k] = tn.x[i][j][k] + dt * (kt1 + 2. * kt2 
                    + 2. * kt3 + kt4) / 6.;
                u.x[i][j][k] = un.x[i][j][k] + dt * (ku1 + 2. * ku2 
                    + 2. * ku3 + ku4) / 6.;
                v.x[i][j][k] = vn.x[i][j][k] + dt * (kv1 + 2. * kv2 
                    + 2. * kv3 + kv4) / 6.;
                w.x[i][j][k] = wn.x[i][j][k] + dt * (kw1 + 2. * kw2 
                    + 2. * kw3 + kw4) / 6.;
                c.x[i][j][k] = cn.x[i][j][k] + dt * (kc1 + 2. * kc2 
                    + 2. * kc3 + kc4) / 6.;
                cloud.x[i][j][k] = cloudn.x[i][j][k] + dt * (kcloud1 
                    + 2. * kcloud2 + 2. * kcloud3 + kcloud4) / 6.;
                ice.x[i][j][k] = icen.x[i][j][k] + dt * (kice1 
                    + 2. * kice2 + 2. * kice3 + kice4) / 6.;
                co2.x[i][j][k] = co2n.x[i][j][k] + dt *(kco1 
                    + 2. * kco2 + 2. * kco3 + kco4) / 6.;
            }
        }
    }
    return;
...
void cAtmosphereModel::solveRungeKutta_2D_Atmosphere(){
    ...
    for(int j = 1; j < jm-1; j++){
        for(int k = 1; k < km-1; k++){
            ...
            kv4 = rhs_v.x[0][j][k];
            kw4 =rhs_w.x[0][j][k];
            v.x[0][j][k] = vn.x[0][j][k] + dt * (kv1 + 2. * kv2 
                + 2. * kv3 + kv4) / 6.;
            w.x[0][j][k] = wn.x[0][j][k] + dt * (kw1 + 2. * kw2 
                + 2. * kw3 + kw4) / 6.;
        }
    }
    return;
