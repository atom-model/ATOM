...
// Runge-Kutta 4. order for k1 step ( dt )
				prepare.RK_RHS_3D_Atmosphere ( n, i, j, k, lv, ls, ep, hp, u_0, t_0, c_0, co2_0, p_0, r_air, r_water, r_water_vapour, r_co2, L_atm, cp_l, R_Air, R_WaterVapour, R_co2, rad, the, phi, h, t, u, v, w, p_dyn, p_stat, c, cloud, ice, co2, tn, un, vn, wn, p_dynn, cn, cloudn, icen, co2n, rhs_t, rhs_u, rhs_v, rhs_w, rhs_p, rhs_c, rhs_cloud, rhs_ice, rhs_co2, aux_u, aux_v, aux_w, Latency, IceLayer, BuoyancyForce, Q_Sensible, P_rain, P_snow, S_v, S_c, S_i, S_r, S_s, S_c_c, Topography );
...
// Runge-Kutta 4. order for k3 step ( dt )
				...
				p_dyn.x[ i ][ j ][ k ] = p_dynn.x[ i ][ j ][ k ] + kp3;
...
void RungeKutta_Atmosphere::solveRungeKutta_2D_Atmosphere ( RHS_Atmosphere &prepare_2D, double r_air, double u_0, double p_0, double L_atm, Array_1D &rad, Array_1D &the, Array &rhs_v, Array &rhs_w, Array &rhs_p, Array &h, Array &v, Array &w, Array &p_dyn, Array &vn, Array &wn, Array &p_dynn, Array &aux_v, Array &aux_w )
{
// Runge-Kutta 4. order for u, v and w component, temperature, water vapour and co2 content
...
// Runge-Kutta 4. order for u, v and w component, temperature and salt concentration
	...
	for ( int j = 1; j < jm-1; j++ )
	{
		...
		{
// Runge-Kutta 4. order for k1 step ( dt )
			prepare_2D.RK_RHS_2D_Atmosphere ( j, k, r_air, u_0, p_0, L_atm, rad, the, h, v, w, p_dyn, vn, wn, p_dynn, rhs_v, rhs_w, rhs_p, aux_v, aux_w );

			kv1 = dt * rhs_v.x[ 0 ][ j ][ k ];
			kw1 = dt * rhs_w.x[ 0 ][ j ][ k ];
			kp1 = dt * rhs_p.x[ 0 ][ j ][ k ];

			v.x[ 0 ][ j ][ k ] = vn.x[ 0 ][ j ][ k ] + kv1 * .5;
			w.x[ 0 ][ j ][ k ] = wn.x[ 0 ][ j ][ k ] + kw1 * .5;
			p_dyn.x[ 0 ][ j ][ k ] = p_dynn.x[ 0 ][ j ][ k ] + kp1 * .5;
	...
	// Runge-Kutta 4. order for k2 step ( dt )
			...
			kp2 = dt * rhs_p.x[ 0 ][ j ][ k ] * .5;

			v.x[ 0 ][ j ][ k ] = vn.x[ 0 ][ j ][ k ] + kv2 * .5;
			w.x[ 0 ][ j ][ k ] = wn.x[ 0 ][ j ][ k ] + kw2 * .5;
			p_dyn.x[ 0 ][ j ][ k ] = p_dynn.x[ 0 ][ j ][ k ] + kp2 * .5;
		...
		// Runge-Kutta 4. order for k3 step ( dt )
			...
			kp3 = dt * rhs_p.x[ 0 ][ j ][ k ] * .5;

			v.x[ 0 ][ j ][ k ] = vn.x[ 0 ][ j ][ k ] + kv3;
			w.x[ 0 ][ j ][ k ] = wn.x[ 0 ][ j ][ k ] + kw3;
			p_dyn.x[ 0 ][ j ][ k ] = p_dynn.x[ 0 ][ j ][ k ] + kp3;
		...
		// Runge-Kutta 4. order for k4 step ( dt )
			...
			if ( p_dyn.x[ 0 ][ j ][ k ] >= 1. )		p_dyn.x[ 0 ][ j ][ k ] = 1.;
			}
		}
	}
