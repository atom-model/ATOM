...
void RungeKutta_Atmosphere::solveRungeKutta_3D_Atmosphere ( RHS_Atmosphere &prepare,
...
// Runge-Kutta 4. order for k1 step ( dt )
                prepare.RK_RHS_3D_Atmosphere ( n, i, j, k, lv, ls, ep, hp, 
                        u_0, t_0, c_0, co2_0, p_0, r_air, r_water_vapour, 
                        r_co2, L_atm, cp_l, R_Air, R_WaterVapour, R_co2,
                        rad, the, phi, h, t, u, v, w, p_dyn, p_stat, c, 
...
// Runge-Kutta 4. order for k4 step ( dt )
                ...
                t.x[ i ][ j ][ k ] = tn.x[ i ][ j ][ k ] 
                    + dt * ( kt1 + 2. * kt2 + 2. * kt3 + kt4 ) / 6.;
                u.x[ i ][ j ][ k ] = un.x[ i ][ j ][ k ] 
                    + dt * ( ku1 + 2. * ku2 + 2. * ku3 + ku4 ) / 6.;
                v.x[ i ][ j ][ k ] = vn.x[ i ][ j ][ k ] 
                    + dt * ( kv1 + 2. * kv2 + 2. * kv3 + kv4 ) / 6.;
                w.x[ i ][ j ][ k ] = wn.x[ i ][ j ][ k ] 
                    + dt * ( kw1 + 2. * kw2 + 2. * kw3 + kw4 ) / 6.;
                c.x[ i ][ j ][ k ] = cn.x[ i ][ j ][ k ] 
                    + dt * ( kc1 + 2. * kc2 + 2. * kc3 + kc4 ) / 6.;
                cloud.x[ i ][ j ][ k ] = cloudn.x[ i ][ j ][ k ] 
                    + dt * ( kcloud1 + 2. * kcloud2 + 2. * kcloud3 + kcloud4 ) / 6.;
                ice.x[ i ][ j ][ k ] = icen.x[ i ][ j ][ k ] 
                    + dt * ( kice1 + 2. * kice2 + 2. * kice3 + kice4 ) / 6.;
                co2.x[ i ][ j ][ k ] = co2n.x[ i ][ j ][ k ] 
                    + dt *( kco1 + 2. * kco2 + 2. * kco3 + kco4 ) / 6.;
            }
        }
    }
