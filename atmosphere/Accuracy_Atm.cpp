...
*
*/
void Accuracy_Atm::steadyQuery_3D (Array &u, Array &un, Array &v, 
    Array &vn, Array &w, Array &wn, Array &t, Array &tn, Array &c, 
    Array &cn, Array &cloud, Array &cloudn, Array &ice, Array &icen, 
    Array &co2, Array &co2n, Array &p_dyn, Array &p_dynn, double L_atm){
    // state of a steady solution (min)
    double min_u = 0.; 
    double min_v = 0.;
    double min_w = 0.;
...
*
*/
void Accuracy_Atm::print(const string& name, double value, int i, int j, 
    int k) const{
    AtomUtils::HemisphereCoords coords = AtomUtils::convert_coords(k, j);
    cout << setiosflags (ios::left) << setw (36) << setfill ('.') 
        << name << " = " << resetiosflags (ios::left) << setw (12) 
        << fixed << setfill (' ') << value << setw (5) << int(coords.lat) 
        << setw (3) << coords.north_or_south << setw (4) << int(coords.lon) 
        << setw (3) << coords.east_or_west << setw (6) << i << setw (2) << "m" << endl;
    return;
}
/*
*
