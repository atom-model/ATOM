                                         ...
                                         Array &cloud, Array &ice )
{
    ...
    for ( int j = 0; j < jm; j++ ){
        for ( int k = 0; k < km; k++ ){//the logic in this loop is confusing.
            ...
            if ( !h.x[ 0 ][ j ][ k ] || !h.x[ 1 ][ j ][ k ] ){
                albedo.y[ j ][ k ] = albedo_val; 
            }
...
void BC_Thermo::Latent_Heat(Array_1D &rad, Array_1D &the, Array_1D &phi, Array &h, Array &t, Array &tn, Array &u, 
                            Array &v, Array &w, Array &p_dyn, Array &p_stat, Array &c, Array &ice, Array &Q_Latent, 
                            Array &Q_Sensible, Array &radiation_3D, Array_2D &Q_radiation, Array_2D &Q_latent,
...
// collection of coefficients
    ...
    for ( int j = 0; j < jm; j++ )
    {
        ...
        for ( int k = 0; k < km; k++ )
        {
            ...
            if ( c.x[ 0 ][ j ][ k ] >= q_Rain ){
                Q_Latent.x[ 0 ][ j ][ k ] = coeff_Lv * a * ( - 3. * c.x[ 0 ][ j ][ k ] + 4. * c.x[ 1 ][ j ][ k ] - 
                                            c.x[ 2 ][ j ][ k ] ) / ( 2. * dr );
            }else{
                Q_Latent.x[ 0 ][ j ][ k ] = 0.;
            }
            if ( c.x[ 0 ][ j ][ k ] >= q_Ice )          
            {
                Q_Latent_Ice = coeff_Ls * a * ( - 3. * ice.x[ 0 ][ j ][ k ] + 4. * ice.x[ 1 ][ j ][ k ] - 
                                ice.x[ 2 ][ j ][ k ] ) / ( 2. * dr );
            }else{
              Q_Latent_Ice = 0.;
            }
            ...
            Q_Sensible.x[ 0 ][ j ][ k ] = coeff_Q * ( - 3. * t.x[ 0 ][ j ][ k ] + 4. * t.x[ 1 ][ j ][ k ] - 
                                          t.x[ 2 ][ j ][ k ] ) / ( 2. * dr )
