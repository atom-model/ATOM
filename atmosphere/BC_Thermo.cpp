...
// boundary condition of surface pressure given by gas equation
    for(int k = 0; k < km; k++){
        for(int j = 0; j < jm; j++){
            ...
            r_humid.x[0][j][k] = 1e2 * p_stat.x[0][j][k]
                /(R_Air * (1. + (R_W_R_A - 1.0) * c.x[0][j][k] 
...
*/
    ...
    double exp_pressure = g/(1e-2 * gam * R_Air);
    for(int k = 0; k < km; k++){
        for(int j = 0; j < jm; j++){
            t_u = t.x[0][j][k] * t_0;
            for(int i = 1; i < im; i++){
                ...
                p_stat.x[i][j][k] = p_stat.x[0][j][k] 
                    * pow(((t_u - gam * height * 1e-2)/(t_u - gam * height_0 * 1e-2)), exp_pressure);
...
//                step[i] = log(((double)i + 1.0)/((double)(i-1) + 1.0));
//                step[i] = log(((double)i + 1.0)/((double)(i-1) + 1.0)) * 400.0; // nan
                step[i] = log((get_layer_height(i+1) + 1.0)
                    /(get_layer_height(i) + 1.0));
...
*
*/
    fft_gaussian_filter_3d(c_u,1);
    fft_gaussian_filter_3d(e_l,1);
    fft_gaussian_filter_3d(g_p,1);
    fft_gaussian_filter_3d(q_v_u,1);
    fft_gaussian_filter_3d(q_c_u,1);
/*
*
*/
    for(int k = 0; k < km; k++){
...
*
*/
    fft_gaussian_filter_3d(q_v_d,1);
    fft_gaussian_filter_3d(e_d,1);
    fft_gaussian_filter_3d(e_p,1);
/*
*
*/
    for(int k = 0; k < km; k++){
