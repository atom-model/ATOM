...
void BC_Thermo::BC_RadiationBalance_parab ( Array_2D &Radiation_Balance_par, Array &h )
{
...
// only switch on, when you know what you do
	...
	for ( int k = 0; k < km; k++ )
	{
		for ( int j = 0; j < jm; j++ )
		{
			for ( int i = 1; i < im; i++ )
			{
//				p_stat.x[ i ][ j ][ k ] = exp ( - g * ( double ) i * ( L_atm / ( double ) ( im-1 ) ) / ( R_Air * t.x[ 0 ][ j ][ k ] * t_0 ) ) * p_0;
