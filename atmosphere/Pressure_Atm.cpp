...
// boundary conditions for the r-direction, loop index i
    ...
    logger() << "enter computePressure_3D: dp_dyn: " << p_dyn.max() * u_0 * u_0 * r_air *.01 << std::endl;
    logger() << "enter computePressure_3D: p_dynn: " << p_dynn.max() * u_0 * u_0 * r_air *.01 << std::endl << std::endl;
...
*/
    logger() << "exit computePressure_3D: p_dyn: " << p_dyn.max() * u_0 * u_0 * r_air *.01 << std::endl;
    logger() << "exit computePressure_3D: p_dynn: " << p_dynn.max() * u_0 * u_0 * r_air *.01 << std::endl << std::endl;
...
// gradients of RHS terms at mountain sides 2.order accurate in the-direction
            ...
            if ( j == 1 )  drhs_vdthe = ( - 3. * aux_v.x[ 0 ][ j ][ k ] + 4. * aux_v.x[ 0 ][ j+1 ][ k ] -
                                                        aux_v.x[ 0 ][ j+2 ][ k ] ) / ( 2. * dthe * rm );

            if ( j == jm-2 )  drhs_vdthe = ( - 3. * aux_v.x[ 0 ][ j ][ k ] + 4. * aux_v.x[ 0 ][ j-1 ][ k ] -
                                                        aux_v.x[ 0 ][ j-2 ][ k ] ) / ( 2. * dthe * rm );
...
// gradients of RHS terms at mountain sides 2.order accurate in phi-direction
            ...
            if ( k == 2 )  drhs_wdphi = ( - 3. * aux_w.x[ 0 ][ j ][ k ] + 4. * aux_w.x[ 0 ][ j ][ k+1 ] -
                                                             aux_w.x[ 0 ][ j ][ k+2 ] ) / ( 2. * rmsinthe * dphi );

            if ( k == km-2 )  drhs_wdphi = ( - 3. * aux_w.x[ 0 ][ j ][ k ] + 4. * aux_w.x[ 0 ][ j ][ k-1 ] -
                                                                 aux_w.x[ 0 ][ j ][ k-2 ] ) / ( 2. * rmsinthe * dphi );
