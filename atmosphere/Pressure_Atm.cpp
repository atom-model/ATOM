...
void cAtmosphereModel::computePressure_3D(){
    ...
    for(int k = 0; k < km; k++){
        for(int i = 0; i < im; i++){
            p_dyn.x[i][0][k] = p_dyn.x[i][0][182];  // along k all values are the same north pole value
            p_dyn.x[i][jm-1][k] = p_dyn.x[i][jm-1][182];  // along k all values are the same south pole value
        }
    }
    ...
    for(int k = 0; k < km; k++){
        for(int j = 1; j < jm-1; j++){
            p_dyn.x[0][j][k] = c43 * p_dyn.x[1][j][k] 
                - c13 * p_dyn.x[2][j][k];  // von Neumann
            p_dyn.x[im-1][j][k] = c43 * p_dyn.x[im-2][j][k] 
                - c13 * p_dyn.x[im-3][j][k];  // von Neumann
        }
    }
