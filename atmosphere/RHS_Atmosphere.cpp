...
void RHS_Atmosphere::RK_RHS_Atmosphere ( int i, int j, int k, double lv, double ls, double ep, double hp, double u_0, double t_0, double t_Boussinesq, double c_0, double co2_0, double p_0, double r_0_air, double r_0_water_vapour, double r_0_co2, double cp_l, double R_Air, double R_WaterVapour, double R_co2, double L_atm, Array_1D &rad, Array_1D &the, Array_1D &phi, Array &h, Array &t, Array &u, Array &v, Array &w, Array &p, Array &c, Array &co2, Array &tn, Array &un, Array &vn, Array &wn, Array &cn, Array &co2n, Array &rhs_t, Array &rhs_u, Array &rhs_v, Array &rhs_w, Array &rhs_c, Array &rhs_co2, Array &aux_u, Array &aux_v, Array &aux_w, Array &Latency, Array &Rain, Array &Ice, Array &Rain_super, Array &IceLayer )
{
// collection of coefficients for phase transformation

	L_atm = L_atm / ( im - 1 );														// characteristic length for non-dimensionalisation

	coeff_lv = ( lv * c_0 ) / ( r_0_air * cp_l * t_0 * u_0 );				// coeff_lv = 0.0078767083
...
// dry adiabatic lapse rate and saturated adiabatic lapse rate = temperature decrease with hight
	...
	p_h = p.x[ i ][ j ][ k ] + exp ( - 9.8066 * (  double ) i * 500. / ( R_Air * t.x[ i ][ j ][ k ] * t_0 ) ) * p_0;	// current air pressure, step size in 500 m, from barometric formula in hPa
...
// SL stands for sea level
	...
	e_SL = ( r_0_water_vapour * R_WaterVapour * t.x[ 0 ][ j ][ k ] * t_0 ) * .01;							// water vapour pressure  in hPa
	a_SL = 216.6 * e_SL / ( t.x[ 0 ][ j ][ k ] * t_0 );																			// absolute humidity in kg/m3
	q_SL = .8 * c.x[ 0 ][ j ][ k ];																										// threshold value for water vapour at sea level
...
// h stands for the local position
	...
	q_h = .8 * c.x[ i ][ j ][ k ];																											// threshold value for water vapour at local hight h
...
//	if ( ( q_h > q_Rain ) && ( i <= 5 ) )	Rain.x[ i ][ j ][ k ] = ( q_h - q_Rain );														// liquid water as surplus of local and saturated water vapour
...
// printout for various thermodynamical quantities for the preticipation computations along the equator ( j = 90 )
	if ( j == 90 )
	{
		...
		cout << " t_h (°C) = " << t_Celsius << "   p_h (hPa) = " << p_h << "   a_h (g/m3) = " << a_h << "   c_h (g/kg) = " << c.x[ i ][ j ][ k ] * 1000. << "   q_Rain (hPa/hPa) = " << q_Rain << "   e_h (hPa) = " << e_h << "   E_Rain (hPa) = " << E_Rain  << endl << endl;
		...
		cout << " t_SL (°C) = " << t_Celsius_SL << "   p_SL (hPa) = " << p_SL << "   a_SL (g/m3) = " << a_SL << "   c_SL (g/kg) = " << c.x[ 0 ][ j ][ k ] * 1000. << "   e_SL (hPa) = " << e_SL << "   q_Ice (hPa/hPa) = " << q_Ice << "   t_tau_SL (°C) = " << t_tau_SL << endl << endl;
...
// force and source terms presented one by one:
	...
	Rain_aux = + WaterVapour * coeff_lv * ( dcdr + dcdthe / rm + dcdphi / rmsinthe ); 									// only the latent heat of water vapour is used!!!!!!!
//	Rain_super_aux = + WaterVapour * coeff_lv * ( dcdr + dcdthe / rm + dcdphi / rmsinthe );
//	Ice_aux = + WaterVapour * coeff_ls * ( dcdr + dcdthe / rm + dcdphi / rmsinthe );
...
//	if ( ( Rain_aux >= 0. ) && ( q_h <= q_Rain ) )	RS_LatentHeat_Energy_Rain = Rain_aux;

	if ( Rain_aux < 0. )		RS_LatentHeat_Energy_Rain = Rain_aux;						// only the latent heat of water vapour is used!!!!!!!
	if ( Rain_aux >= 0. )	RS_LatentHeat_Energy_Rain = Rain_aux;
...
// Right Hand Side of the time derivative ot temperature, pressure, water vapour concentration and velocity components
	...
	rhs_t.x[ i ][ j ][ k ] = - ( u.x[ i ][ j ][ k ] * dtdr + v.x[ i ][ j ][ k ] * dtdthe / rm + w.x[ i ][ j ][ k ] * dtdphi / rmsinthe )
			...
			+ h_d_i * gr * u.x[ i ][ j ][ k ] * ( t.x[ i ][ j ][ k ] - 2. * t_Boussinesq ) / t_Boussinesq
...
//			- RS_LatentHeat_Energy_Rain - RS_LatentHeat_Energy_Rain_super - RS_LatentHeat_Energy_Ice;
	...
	rhs_u.x[ i ][ j ][ k ] = - ( u.x[ i ][ j ][ k ] * dudr + v.x[ i ][ j ][ k ] * dudthe / rm + w.x[ i ][ j ][ k ] * dudphi / rmsinthe )
			- dpdr + h_d_i * gr * ( t.x[ i ][ j ][ k ] - 2. * t_Boussinesq ) / t_Boussinesq
...
void RHS_Atmosphere::RK_RHS_2D_Atmosphere ( int j, int k, Array_1D &rad, Array_1D &the, Array_1D &phi, Array &h, Array &t, Array &v, Array &w, Array &p, Array &tn, Array &vn, Array &wn, Array &rhs_t, Array &rhs_v, Array &rhs_w, Array &aux_v, Array &aux_w )
{
...
//		dvdr = h_d_i * ( 4. * v.x[ i + 1 ][ j ][ k ] - v.x[ i + 2 ][ j ][ k ] ) / ( 2. * dr );
//		dwdr = h_d_i * ( 4. * w.x[ i + 1 ][ j ][ k ] - w.x[ i + 2 ][ j ][ k ] ) / ( 2. * dr );
		dvdr = 0.;
		dwdr = 0.;
		dtdr = 0.;
		...
		dtdthe = h_d_j * ( t.x[ 0 ][ j+1 ][ k ] - t.x[ 0 ][ j-1 ][ k ] ) / ( 2. * dthe );
		...
		dtdphi = h_d_k * ( t.x[ 0 ][ j ][ k+1 ] - t.x[ 0 ][ j ][ k-1 ] ) / ( 2. * dphi );

//		d2vdr2 = h_d_i * ( - 2. * v.x[ i + 1 ][ j ][ k ] + v.x[ i + 2 ][ j ][ k ] ) / dr2;
//		d2wdr2 = h_d_i * ( - 2. * w.x[ i + 1 ][ j ][ k ] + w.x[ i + 2 ][ j ][ k ] ) / dr2;
		d2vdr2 = 0.;
		d2wdr2 = 0.;
		d2tdr2 = 0.;
		...
		d2tdthe2 = h_d_j * ( t.x[ 0 ][ j+1 ][ k ] - 2. * t.x[ 0 ][ j ][ k ] + t.x[ 0 ][ j-1 ][ k ] ) / dthe2;

		d2vdphi2 = h_d_k * ( v.x[ 0 ][ j ][ k+1 ] - 2. * v.x[ 0 ][ j ][ k ] + v.x[ 0 ][ j ][ k-1 ] ) / dphi2;
		d2wdphi2 = h_d_k * ( w.x[ 0 ][ j ][ k+1 ] - 2. * w.x[ 0 ][ j ][ k ] + w.x[ 0 ][ j ][ k-1 ] ) / dphi2;
		d2tdphi2 = h_d_k * ( t.x[ 0 ][ j ][ k+1 ] - 2. * t.x[ 0 ][ j ][ k ] + t.x[ 0 ][ j ][ k-1 ] ) / dphi2;


		RS_Coriolis_Energy = ( - w.x[ i ][ j ][ k ] * coriolis * ( 2. * omega * costhe * v.x[ i ][ j ][ k ] )
												+ v.x[ i ][ j ][ k ] * coriolis * 2. * omega * costhe * w.x[ i ][ j ][ k ] ) * ec * pr;
		RS_centrifugal_Energy = + centrifugal * rad.z[ 0 ] * pow ( ( omega * sinthe ), 2 ) * ec * pr
													+ centrifugal * rad.z[ 0 ] * sinthe * costhe * pow ( ( omega ), 2 ) * ec * pr;
		...
		rhs_t.x[ 0 ][ j ][ k ] = - ( v.x[ 0 ][ j ][ k ] * dtdthe / rm + w.x[ 0 ][ j ][ k ] * dtdphi / rmsinthe )
				+ ec * ( v.x[ 0 ][ j ][ k ] / rm * dpdthe + w.x[ 0 ][ j ][ k ] / rmsinthe * dpdphi )
				+ ( d2tdr2 + dtdr * 2. / rm + d2tdthe2 / rm2 + dtdthe * costhe / rm2sinthe + d2tdphi2 / rm2sinthe2 ) / ( re * pr )
				+ 2. * ec / re * ( pow ( ( dvdthe / rm ), 2. )
				+ pow ( ( dwdphi / rmsinthe + v.x[ 0 ][ j ][ k ] * cotthe / rm ), 2. ) )
				+ ec / re * ( pow ( ( dvdr - v.x[ 0 ][ j ][ k ] / rm ), 2. )
				+ pow ( ( dwdr - w.x[ 0 ][ j ][ k ] / rm ), 2. )
				+ pow ( ( dwdthe * sinthe / rm2 - w.x[ 0 ][ j ][ k ] * costhe / rmsinthe + dvdphi / rmsinthe ), 2. ) )
				+ RS_Coriolis_Energy + RS_centrifugal_Energy
				- RS_LatentHeat_Energy_Rain; 																						// only the latent heat of water vapour is used!!!!!!!
		...
		rhs_w.x[ 0 ][ j ][ k ] = - ( v.x[ 0 ][ j ][ k ] * dwdthe / rm +  w.x[ 0 ][ j ][ k ] * dwdphi / rmsinthe ) +
				...
				- ( 1. + costhe * costhe / sinthe2 ) * h_d_k * w.x[ 0 ][ j ][ k ] / dthe2 / rm + d2wdphi2 / rm2sinthe2 + dvdphi * 2. * costhe / rm2sinthe2 ) / re
				+ RS_Coriolis_Momentum_phi
